...
**Лента важного** - закреплённое и подсвеченное правилами (по желанию и упоминания) уходит из колонок в
отдельную строку прямо над доской: закреплённое стоит слева, новое важное плавно въезжает справа. Карточки
те же, что в колонках, с теми же кнопками и с названием источника; не влезают в строку - по краям стрелки.
Несколько важных из одного чата собираются в одну плашку, и она раскрывается вверх. Лента ездит за доской,
а если сверху мало места, встаёт под ней. Включается в настройках, «Внешний вид» - «Лента важного».
На Linux пустые места строки прозрачны и пропускают клики к окнам под ней; на Windows и Mac лента - полоса
высотой по содержимому.

![Лента важного над доской](docs/screens/ru/widget-strip.png)
...
- **Контейнеры** - Docker и Podman: контейнер упал, не хватило памяти, healthcheck «нездоров»,
  ...
  останавливает. На Mac находит Docker Desktop, OrbStack, Colima, Rancher Desktop и Podman, даже когда
  их нет в PATH программы.
...
- **«Думающие» модели** (qwen3, deepseek-r1 и подобные) - размышления свёрнуты над ответом. На них
  уходит остаток контекста после выборки; если модель истратит его целиком, она сразу ответит ещё раз,
  без размышлений. Таким моделям лучше контекст от 16 384. У облачных моделей контекст - самой модели,
  а сколько сообщений отдать, решает «Сообщений в выборке»; сколько не влезло, видно в сводке выборки.
...
скрыть, подсветить цветом, перенести в свою колонку, сразу считать прочитанным, закрепить, проиграть
звук, переслать в Telegram.

![Редактор правила](docs/screens/ru/settings-rule-editor.png)

**Своя колонка** - действие «Перенести в колонку»: название (например, «Личное») и значок на выбор.
Сообщения по правилу уходят из колонки своего приложения в эту колонку - так в одну колонку собираются
домашние чаты из Telegram, WhatsApp и MAX. У каждой карточки маленький значок, откуда она пришла;
колонку можно разделить на подколонки по приложениям, закрыть или скрыть, как любую другую. В своём
источнике условие можно не задавать - тогда в колонку переносится весь источник.

![Правило «Перенести в колонку»](docs/screens/ru/settings-rule-move.png)

![Своя колонка «Личное» на доске](docs/screens/ru/widget-mycol.png)
...
**Внешний вид** - язык, прозрачность, размер текста, тема, плотность, аватары, лента важного.
