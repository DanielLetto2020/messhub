...
- **«Спросить свои сообщения».** Ассистент на модели на твоём же компьютере: «что я пропустил?»,
  «кто спрашивал про отчёт?», «сделай сводку по чату» - с номерами сообщений в ответе.
...
**Тихие часы** - по расписанию (интервал может идти через полночь), вручную из шапки доски («тихо на
час», «до утра») или вместе с «Не беспокоить» GNOME. Пока тихо, правила не играют звук, а потом
приходит одна сводка: сколько пришло и откуда. По желанию программа сама включает «Не беспокоить».
...
- Ассистент отвечает моделью на этом компьютере (Ollama или LM Studio). Интернет нужен, только
  чтобы один раз скачать модель; адрес модели в облаке программа не примет.
